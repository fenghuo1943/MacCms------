...
class SeleniumDoubanFetcher:
    ...
    def check_stop_condition(self) -> bool:
        ...
        if os.path.exists(self.stop_flag_file):
            ...
            try:
                ...
                main_logger.info(f"已将停止文件重命名为: {backup_file}")
            except Exception as e:
                main_logger.warning(f"重命名停止文件失败: {e}")
            return True
    ...
    def process_single_video(self, video: Dict) -> Tuple[int, bool, str]:
        ...
        try:
            ...
            # 2. 匹配视频（使用DataProcessor，传入api_client）
    ...
    def run(self, batch_size: int = 50):
        ...
        try:
            while True:
                ...
                for i, video in enumerate(videos, 1):
                    # 在处理每个视频前检查停止条件
                    if self.check_stop_condition():
                        main_logger.info(f"在处理第 {i}/{len(videos)} 个视频时收到停止请求")
                        break
        ...
        if self.stop_requested or os.path.exists(self.stop_flag_file):
            main_logger.info("任务已优雅停止！")
        else:
            main_logger.info("任务完成！")
        main_logger.info(f"总处理: {total_processed} 个")
        main_logger.info(f"成功: {total_success} 个 ({total_success/total_processed*100:.1f}%)" if total_processed > 0 else "成功: 0 个")
        main_logger.info(f"总耗时: {self.format_eta(total_elapsed)}")
        main_logger.info(f"平均速度: {total_processed/total_elapsed:.2f} 个/秒" if total_elapsed > 0 else "平均速度: 0 个/秒")
        main_logger.info("=" * 70)
