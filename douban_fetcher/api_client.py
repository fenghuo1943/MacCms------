...
class ApiClient:
    ...
    def search_douban(self, video_name: str, monitor=None, max_retries: int = 5) -> Optional[List[Dict]]:
        """
        使用豆瓣搜索API搜索视频
        ...
        Returns:
            搜索结果列表，失败返回None
        """
        url = DOUBAN_API_CONFIG['search_url']
        ...
        for attempt in range(max_retries):
            try:
                ...
                data = {
                    'q': video_name,
                    'apikey': DOUBAN_API_CONFIG['apikey']
                }
                ...
                if response.status_code == 429:
                    wait_time = base_delay * (2 ** attempt) + random.uniform(0, 1)
                    logger.warning(f"豆瓣搜索API触发频率限制，等待 {wait_time:.1f} 秒")
                    if monitor:
                        monitor.record_request(False, rate_limited=True)
                    time.sleep(wait_time)
                    continue
    ...
    def get_douban_subject(self, douban_id: str, monitor=None, max_retries: int = 5) -> Optional[Dict]:
        """
        通过豆瓣ID从豆瓣Subject API获取详细信息
        ...
        for attempt in range(max_retries):
            try:
                ...
                if response.status_code == 429:
                    wait_time = base_delay * (2 ** attempt) + random.uniform(0, 1)
                    logger.warning(f"豆瓣Subject API触发频率限制，等待 {wait_time:.1f} 秒")
                    if monitor:
                        monitor.record_request(False, rate_limited=True)
                    time.sleep(wait_time)
                    continue
