"""
测试豆瓣API搜索功能的POST方法和不同参数格式
"""
import requests
import json


def test_douban_search_post():
    """测试豆瓣API搜索功能的POST方法"""
    ...
    test_cases = [
        {
            'name': '直接中文 (POST)',
            'data': {'q': '爱情公寓', 'apikey': api_key}
        },
        {
            'name': 'URL编码中文 (POST)',
            'data': {'q': '%E7%88%B1%E6%83%85%E5%85%AC%E5%AF%93', 'apikey': api_key}
        },
        {
            'name': 'Unicode转义 (POST)',
            'data': {'q': '\\u7231\\u60c5\\u516c\\u5bd3', 'apikey': api_key}
        }
    ]
...
def test_correct_format():
    ...
    try:
        # 使用GET方法，requests会自动处理URL编码
        response = requests.get(base_url, params=params, headers=headers, timeout=10)
        print(f"GET方法 - 状态码: {response.status_code}")
        if response.status_code == 200:
            data = response.json()
            print(f"返回标题: {data.get('title', 'N/A')}")
        else:
            print(f"GET失败: {response.text[:200]}")
    except Exception as e:
        print(f"GET错误: {str(e)}")
    ...
    try:
        ...
        print(f"POST方法 - 状态码: {response.status_code}")
        if response.status_code == 200:
            data = response.json()
            print(f"返回标题: {data.get('title', 'N/A')}")
        else:
            print(f"POST失败: {response.text[:200]}")
    except Exception as e:
        print(f"POST错误: {str(e)}")


if __name__ == '__main__':
    test_douban_search_post()
    test_correct_format()